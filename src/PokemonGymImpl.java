...
public class PokemonGymImpl implements PokemonGym {
    ...
    public void fightRound(PokemonTrainer trainer, PokemonGymOwner owner, Pokemon pokemon, Pokemon gymPokemon) {
        ...
        while (pokemon.getHp() > 0 && gymPokemon.getHp() > 0) {
            ...
            attackOrChangeOrFeed(pokemon, gymPokemon, trainer, owner);
    ...
    // toegevoegd voor bonusopdracht 1
    public void performFeedPlayer(Pokemon pokemon) {
        pokemon.setHp(pokemon.getHp() + 10);
        System.out.println(pokemon.getName() + " is fed with " + pokemon.getFood());
        System.out.println(pokemon.getName() + " gains 10 hp");
        System.out.println(pokemon.getName() + " now has an hp of: " + pokemon.getHp());
    }

    @Override
    public void attackOrChangeOrFeed(Pokemon pokemon, Pokemon gymPokemon, PokemonTrainer trainer, PokemonGymOwner gym) {
        Scanner speler_A = new Scanner(System.in);
        ...
        if (choice.equalsIgnoreCase("a")) {
            ...
            attackOrChangeOrFeed(pokemon, gymPokemon, trainer, gym);
            fightRound(trainer, gym, pokemon, gymPokemon);
        } else {
            performFeedPlayer(pokemon);
        }
    }
